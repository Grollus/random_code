...
library(data.table)
# get the input data
data <- fread("crime_addresses.csv", data.table = FALSE)
names(data) <- c("id", "address")
...
addresses = paste0(addresses, ", Louisville")
...
saveRDS(data, paste0("../data/", input ,"_geocoded.rds"))
write.table(data, file=paste0("../data/", input ,"_geocoded.csv"), sep=",", row.names=FALSE)