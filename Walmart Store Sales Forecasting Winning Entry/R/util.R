require(plyr)
setwd("D:/RProgram/random_code/Walmart Store Sales Forecasting Winning Entry")

#TODO: set data, submit and r paths to correct directories, if needed
paths = list(data = paste0(getwd(),'/data/'),
             submit = paste0(getwd(), '/submissions/'),
             r = paste0(getwd(),'/R/'))
...
raw.train <- function(){
  # Load training data with correct classes
  # Had to sift through first to figure out what class each column should be
...
raw.test <- function(){
  # Load the test data with correct classes
...
wmae <- function(pred, test){
  # Computes the evaluation metric for competition
  #
  #args:
  # pred - a data frame with predictions in the Weekly_Sales field
